...
function setSectionIntoViewActive(sections) {
  for (const section of sections) {
    ...
    const activeeddieLink = document.querySelector(
    ...
    if (isTopSectionInViewport(section)) {
      ...
      section.classList.add("activeeddie");
      activeeddieLink.classList.add("menu__link--activeeddie");
    } else {
      // if it is not, remove active styles
      section.classList.remove("activeeddie");
      activeeddieLink.classList.remove("menu__link--activeeddie");
    }
  }
}
